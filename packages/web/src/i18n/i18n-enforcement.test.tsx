...
function PseudoLocaleFixture() {
    return <IntlProvider locale="en-XA" messages={pseudoMessages} onError={(error) => {
        ...
        <ArticleHeader article={createArticle()} updateArticle={vi.fn()} save={vi.fn()} remove={vi.fn()} focusMode={false} setFocusMode={vi.fn()} />
        <ArticleStatusBar revisionNumber={1} language="en" setLanguage={vi.fn()} saveState="saved" length={{ count: 12, remaining: 2988, state: "within-limit" }} profile={publishLimitProfiles[1]!} customProfiles={[]} setProfile={vi.fn()} copyMarkdown={vi.fn()} copyPlainText={vi.fn()} />
