...
export function GeneralSettingsSection({ general, save, applyTheme }: { general: GeneralSettings; save: (next: GeneralSettings) => Promise<void>; applyTheme?: (theme: GeneralSettings["theme"]) => void }) {
    ...
    return <>
        <SettingRow label={intl.formatMessage({ id: "settings.preferredAppearance" })} hint={intl.formatMessage({ id: "settings.appearanceHint" })} action={<Button variant="quiet" onClick={() => applyTheme?.(general.theme)}>{intl.formatMessage({ id: "settings.applyAppearance" })}</Button>}><Select value={general.theme} onChange={(event) => {
            const value = event.target.value;
            if (isThemePreference(value))
                void save({ ...general, theme: value });
        }}><option value="system">{intl.formatMessage({ id: "settings.system" })}</option><option value="light">{intl.formatMessage({ id: "settings.light" })}</option><option value="dark">{intl.formatMessage({ id: "settings.dark" })}</option></Select></SettingRow>
        <SettingRow label={intl.formatMessage({ id: "settings.interfaceLanguage" })} hint={intl.formatMessage({ id: "settings.interfaceLanguageHint" })}><Select value={catalogByLocale.has(general.interfaceLocale) ? general.interfaceLocale : "en"} disabled={installedLocaleCatalogs.length === 1} onChange={(event) => {
            const value = event.target.value;
            if (isInterfaceLocale(value))
                void save({ ...general, interfaceLocale: value });
        }}>{installedLocaleCatalogs.map((catalog) => <option key={catalog.code} value={catalog.code}>{intl.formatMessage({ id: catalog.nameMessageId })}</option>)}</Select></SettingRow>
        ...
        }}><option value="system">{intl.formatMessage({ id: "settings.systemDateFormat" }, { value: formatSystemDateExample(general) })}</option><option value="day-first">{intl.formatMessage({ id: "settings.dayFirstSlash" })}</option><option value="day-first-dots">{intl.formatMessage({ id: "settings.dayFirstDots" })}</option><option value="month-first">{intl.formatMessage({ id: "settings.monthFirstSlash" })}</option><option value="iso">{intl.formatMessage({ id: "settings.isoDate" })}</option></Select></SettingRow>
