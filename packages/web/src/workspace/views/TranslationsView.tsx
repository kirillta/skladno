...
export function TranslationsView({ article, translation, stale, create }: {
    ...
    const intl = useIntl();
    return <div>
        <h2 className="font-semibold">{intl.formatMessage({ id: "views.translations" })}</h2>
        {article.sourceArticleId && <p>{intl.formatMessage({ id: "views.sourceLinked" }, { revisionId: article.sourceRevisionId?.slice(0, 8) })}</p>}
