...
export function PublishingPreviewView({ publishing }: {
    publishing: {
        ...
        settings: { customProfiles: { id: `custom-${string}`; name: string }[] };
    ...
    const label = (id: PublishLimitProfileId) => id === PUBLISH_LIMIT_PROFILE.NO_RESTRICTIONS ? intl.formatMessage({ id: "publishing.noRestrictions" }) : id === PUBLISH_LIMIT_PROFILE.LINKEDIN_POST ? intl.formatMessage({ id: "publishing.linkedInPost" }) : id === PUBLISH_LIMIT_PROFILE.LINKEDIN_ARTICLE ? intl.formatMessage({ id: "publishing.linkedInArticle" }) : publishing.settings.customProfiles.find((profile) => profile.id === id)?.name ?? intl.formatMessage({ id: "publishing.default" });
    return <div className="mx-auto w-full max-w-3xl p-5">
        ...
        <Select className="mt-3" aria-label={intl.formatMessage({ id: "settings.publishingProfile" })} value={publishing.profileId} onChange={(event) => void publishing.setProfile(event.target.value as PublishLimitProfileId)}>
            {[...publishLimitProfiles, ...publishing.settings.customProfiles].map((profile) => <option key={profile.id} value={profile.id}>{label(profile.id)}</option>)}
        </Select>
