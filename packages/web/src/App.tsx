...
export function App() {
    ...
    return <main className={`grid min-h-screen grid-cols-1 bg-canvas font-ui text-ink md:h-screen ${distractionFree ? "md:grid-cols-[minmax(0,1fr)]" : assistantCollapsed ? navigationCollapsed ? "md:grid-cols-[4rem_minmax(0,1fr)]" : "md:grid-cols-[14rem_minmax(0,1fr)]" : navigationCollapsed ? "md:grid-cols-[4rem_minmax(0,1fr)_20rem]" : "md:grid-cols-[14rem_minmax(0,1fr)_20rem]"}`}>
        <aside className={`${distractionFree ? "hidden" : ""} overflow-y-auto border-b border-border bg-surface py-4 md:border-r md:border-b-0`} aria-label="Document navigation">
            <div className="flex items-center justify-between gap-2 border-b border-border px-4 pb-4"><h1 className={navigationCollapsed ? "sr-only" : "text-sm font-semibold tracking-tight text-brand"}>✦&nbsp; Skladno</h1><Button type="button" variant="quiet" className="min-h-8 px-2 py-1" aria-label={navigationCollapsed ? "Expand navigation" : "Collapse navigation"} onClick={toggleNavigation}>{navigationCollapsed ? ">" : "‹"}</Button></div>
            <div className="flex items-center justify-between px-3 pt-3"><span className={navigationCollapsed ? "sr-only" : "text-[0.7rem] font-medium uppercase tracking-[0.12em] text-muted"}>Workspace</span><Button type="button" variant="quiet" className="min-h-8 px-2 py-1 text-base" aria-label="Create document" onClick={() => setIsCreateDialogOpen(true)}>+</Button></div>
            {!navigationCollapsed && <>
                ...
                <div className="mt-5 border-t border-border pt-3"><button type="button" className="w-full px-4 py-2 text-left text-xs text-muted hover:bg-brand-soft">Style profile</button><button type="button" className="w-full px-4 py-2 text-left text-xs text-muted hover:bg-brand-soft">Settings</button><p className="px-4 pt-2 text-[0.65rem] text-muted">EN · Local <span className="float-right text-success">● Saved</span></p></div>
            </>}
            {!navigationCollapsed && <>
