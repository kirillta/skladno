...
const ui = {
    primaryButton:
        "rounded-md bg-brand px-2.5 py-1.5 text-xs font-medium text-white transition hover:bg-brand/90 disabled:cursor-not-allowed disabled:opacity-55",
    secondaryButton:
        "rounded-md border border-brand/60 px-2.5 py-1.5 text-xs font-medium text-brand transition hover:bg-brand-soft disabled:cursor-not-allowed disabled:opacity-55",
    ...
    navigationIconButton: "size-9 min-h-9 shrink-0 p-0 leading-none",
...
function revisionKind(version: DocumentVersion): string {
    return typeof version.provenance.kind === "string" ? version.provenance.kind : "saved-revision";
}


function revisionMarker(version: DocumentVersion): string {
    const kind = revisionKind(version);

    if (kind === "accepted-proposal") return "✦";

    if (kind === "author-draft") return "✎";
...
export function App() {
    ...
    const [restoreCandidate, setRestoreCandidate] = useState<DocumentVersion>();
    ...
    useEffect(() => {
        ...
        client
            .getStyleCorpus()
            .then(setStyleCorpus)
            .catch(() => setStyleCorpusMessage("Couldn’t load your local style corpus."));
        client
            .getPublishLimitProfile()
            .then(setPublishLimitProfileId)
            .catch(() => setPublishMessage("Using the default publishing profile until local settings are available."));
    }, []);
    ...
    const review: TextProposal | undefined = proposalBase ? createTextProposal(proposalBase.content, proposal) : undefined;
    ...
    const visibleDocuments = documents.filter((document) =>
        `${document.title} ${document.language ?? ""} ${document.audience ?? ""}`.toLocaleLowerCase().includes(libraryQuery.trim().toLocaleLowerCase()),
    );
    const viewedVersion =
        history.find((version) => version.id === viewedVersionId) ??
        history.find((version) => version.id === selected?.currentVersionId) ??
    ...
    function loadHistory(documentId: string) {
        client
            ...
            .then(setHistory)
            .catch(() => setHistoryMessage("Couldn’t load version history."));
    }
            ...
            items.map((item) =>
                item.id === documentId
                    ? {
                          ...
                          currentVersion: { ...item.currentVersion, id: versionId, content: savedContent },
                      }
                    : item,
            ),
        );
    ...
    async function createDocumentFromFlow(startFromPoints: boolean) {
        ...
        try {
            ...
            if (startFromPoints) void requestEditorialProposal(EDITORIAL_OPERATION.THESIS_TO_NARRATIVE, created, points);
            else setEditorialMessage("Empty draft created locally.");
        } catch {
            setSaveState("error");
        }
    }
    ...
    async function deleteArticle(document: Document) {
        ...
        try {
            ...
            setDrafts((items) => {
                const { [document.id]: _, ...remaining } = items;
                return remaining;
            });
    ...
    async function requestEditorialProposal(operation: EditorialOperation, document = selected, context = editorialContext) {
        ...
        try {
                    ...
                    if (event.type === "text_delta") {
                        ...
                        setEditorialMessage(
                            event.factCheck
                                ...
                                : "Proposal ready for review. It has not changed your article.",
                        );
                    } else {
                        setEditorialState(EditorialState.Error);
            ...
            if (editorialRequest.current === controller) editorialRequest.current = undefined;
        }
    }
    ...
    async function acceptProposal(changeIds = selectedChanges) {
        if (!selected || !proposalBase || !review || changeIds.size === 0) return;
        ...
        try {
            ...
            setEditorialState(EditorialState.Error);
            setEditorialMessage(
                error instanceof Error ? `${error.message} Review the proposal again against the current article.` : "Couldn’t accept this proposal.",
            );
        }
    }

    async function acceptTranslation() {
        if (!selected || !translation || !proposal.trim()) return;
    ...
    if (state !== "ready")
        return (
            <main className="min-h-screen bg-canvas px-8 pt-[12vh] font-ui text-ink">
                <div className="mx-auto max-w-2xl">
                    <h1 className="text-lg font-semibold tracking-tight">Skladno</h1>
                    <p className="mt-2 text-sm text-muted" aria-live="polite" data-state={state}>
                        {message}
                    </p>
                </div>
            </main>
    ...
    return (
        <main
            className={`grid min-h-screen grid-cols-1 bg-canvas font-ui text-ink md:h-screen ${distractionFree ? "md:grid-cols-[minmax(0,1fr)]" : assistantCollapsed ? (navigationCollapsed ? "md:grid-cols-[2.75rem_minmax(0,1fr)]" : "md:grid-cols-[14rem_minmax(0,1fr)]") : navigationCollapsed ? "md:grid-cols-[2.75rem_minmax(0,1fr)_20rem]" : "md:grid-cols-[14rem_minmax(0,1fr)_20rem]"}`}
        >
            <aside
                className={`${distractionFree ? "hidden" : ""} flex overflow-x-hidden overflow-y-auto border-b border-border bg-surface py-4 md:flex-col md:border-r md:border-b-0`}
                aria-label="Document navigation"
            >
                <div className={`flex items-center border-b border-border pb-4 ${navigationCollapsed ? "justify-center px-1" : "justify-between px-3"}`}>
                    <h1 className={navigationCollapsed ? "sr-only" : "text-sm font-semibold tracking-tight text-brand"}>✦&nbsp; Skladno</h1>
                    <Button
                        type="button"
                        variant="quiet"
                        className={navigationCollapsed ? `${ui.navigationIconButton} font-semibold text-brand` : `${ui.navigationIconButton} text-2xl`}
                        aria-label={navigationCollapsed ? "Expand navigation" : "Collapse navigation"}
                        onClick={toggleNavigation}
                    >
                        {navigationCollapsed ? "S" : "‹"}
                    </Button>
                </div>
                <div className={`flex px-3 pt-3 ${navigationCollapsed ? "justify-center" : "items-center justify-between"}`}>
                    <span className={navigationCollapsed ? "sr-only" : "text-[0.7rem] font-medium uppercase tracking-[0.12em] text-muted"}>Workspace</span>
                    <Button
                        type="button"
                        variant="quiet"
                        className={`${ui.navigationIconButton} text-base`}
                ...
                {!navigationCollapsed && (
                    <>
                        <label className="mx-3 mt-3 grid gap-1 text-xs text-muted">
                            <span className="sr-only">Search documents</span>
                            <Field
                                className="min-h-8 bg-surface-raised px-2 py-1.5 text-xs"
                        ...
                        <section className="mt-4" aria-label="Recent documents">
                            <h2 className="px-4 text-[0.65rem] font-semibold uppercase tracking-[0.1em] text-muted">Recent</h2>
                            {documents.length === 0 ? (
                                <EmptyState title="No documents yet">
                                    <Button type="button" variant="secondary" onClick={() => setIsCreateDialogOpen(true)}>
                                        Create your first document
                                    </Button>
                                </EmptyState>
                            ) : visibleDocuments.length === 0 ? (
                                <EmptyState title="No matching documents">
                                    <Button type="button" variant="quiet" onClick={() => setLibraryQuery("")}>
                                        Clear search
                                    </Button>
                                </EmptyState>
                            ) : (
                                <ul className="m-0 mt-2 list-none space-y-1 p-0">
                                    {visibleDocuments.map((document) => (
                                        <li key={document.id} className={`mx-2 rounded-control p-0.5 ${document.id === selectedId ? "bg-brand-soft" : ""}`}>
                                            {renameId === document.id ? (
                                                <input
                                                    ...
                                                    onKeyDown={(event) => {
                                                        if (event.key === "Enter") void commitRename(document.id);
                                                ...
                                                <button
                                                    type="button"
                                                    className="block w-full truncate rounded px-2 py-2 text-left text-sm hover:bg-white/45"
                                                    onClick={() => selectArticle(document.id)}
                                                >
                                                    {document.title}
                                                    <small>
                                                        {document.sourceDocumentId
                                                            ? `Linked translation · ${document.language ?? "language not set"}`
                                                            : `${document.language ?? "Language not set"} · ${new Date(document.updatedAt).toLocaleDateString()}`}
                                                    </small>
                                                </button>
                                            )}
                                            <div className="hidden">
                                                ...
                                                <button type="button" onClick={() => void deleteArticle(document)}>
                                                    Delete
                                                </button>
                                            </div>
                                        </li>
                        ...
                        <div className="mt-auto border-t border-border pt-3">
                            <button type="button" className="w-full px-4 py-2 text-left text-xs text-muted hover:bg-brand-soft">
                            ...
                            <p className="px-4 pt-2 text-[0.65rem] text-muted">EN</p>
                        </div>
                    </>
                )}
                {navigationCollapsed && (
                    <div className="mt-auto grid justify-items-center gap-1 border-t border-border py-3">
                        <Button
                            type="button"
                            variant="quiet"
                            className={`${ui.navigationIconButton} text-base`}
                        ...
                        <Button type="button" variant="quiet" className={`${ui.navigationIconButton} text-base`} aria-label="Settings" title="Settings">
                            ⚙
                        </Button>
                        <span className="pt-1 text-[0.65rem] text-muted" aria-label="English">
                            EN
                        </span>
                    </div>
                )}
                        ...
                        <div className="hidden">
                            <section className={ui.sidebarSection} aria-label="Publish mode">
                                ...
                                <label>
                                    Length guidance
                                    <Select
                                        aria-label="Publishing length profile"
                                        value={publishLimitProfileId}
                                        onChange={(event) => void selectPublishLimitProfile(event.target.value as PublishLimitProfileId)}
                                    >
                                        {publishLimitProfiles.map((profile) => (
                                            <option key={profile.id} value={profile.id}>
                                                {profile.label} ({profile.characterLimit.toLocaleString()} characters)
                                            </option>
                                        ))}
                                    </Select>
                                </label>
                                <p
                                    className={`text-xs ${publishCharacterCount > publishLimitProfile.characterLimit ? "text-danger" : publishCharacterCount >= publishLimitProfile.warningThreshold ? "text-caution" : "text-brand"}`}
                                    aria-live="polite"
                                >
                                    {publishCharacterCount.toLocaleString()} / {publishLimitProfile.characterLimit.toLocaleString()} characters
                                    {publishCharacterCount > publishLimitProfile.characterLimit
                                        ? " — over this profile’s guidance."
                                        : publishCharacterCount >= publishLimitProfile.warningThreshold
                                          ? " — nearing this profile’s guidance."
                                          : " — within this profile’s guidance."}
                                </p>
                                <Button type="button" onClick={() => void copyPublishingText()} disabled={!selected}>
                                    Copy plain text
                                </Button>
                                <p className="publish-format-note">
                                    Links are kept as readable text. LinkedIn may not preserve every line break or other formatting exactly after paste.
                                </p>
                                <output
                                    className="mt-2 block max-h-64 overflow-y-auto rounded-md border border-border bg-canvas p-2 font-editor text-xs leading-5 whitespace-pre-wrap select-text"
                                    aria-label="Plain-text publishing preview"
                                >
                                    {publishText || "Your plain-text preview will appear here."}
                                </output>
                            ...
                            <section className={ui.sidebarSection} aria-label="Editorial assistant">
                                ...
                                <TextareaField
                                    aria-label="Theses or editorial guidance"
                                    value={editorialContext}
                                    onChange={(event) => setEditorialContext(event.target.value)}
                                    placeholder="Add theses, a tone, or revision guidance…"
                                    disabled={!selected || editorialState === EditorialState.Streaming}
                                />
                                <div className="mt-2 flex flex-wrap gap-2">
                                    <button
                                        type="button"
                                        onClick={() => void requestEditorialProposal(EDITORIAL_OPERATION.THESIS_TO_NARRATIVE)}
                                        disabled={!selected || editorialState === EditorialState.Streaming}
                                    >
                                        Turn theses into narrative
                                    </button>
                                    <button
                                        type="button"
                                        ...
                                        disabled={!selected || editorialState === EditorialState.Streaming}
                                    >
                                        Revise draft for flow
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => void requestEditorialProposal(EDITORIAL_OPERATION.FACT_CHECK)}
                                        disabled={!selected || editorialState === EditorialState.Streaming}
                                    >
                                        Check facts
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => void requestEditorialProposal(EDITORIAL_OPERATION.STYLE_REVIEW)}
                                        disabled={!selected || editorialState === EditorialState.Streaming || !styleCorpus?.profile}
                                    >
                                        Check style
                                    </button>
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => void requestEditorialProposal(EDITORIAL_OPERATION.TRANSLATION)}
                                        disabled={!selected || editorialState === EditorialState.Streaming || !targetLanguage.trim()}
                                    >
                                        Translate article
                                    </button>
                                    {editorialState === EditorialState.Streaming && (
                                        <button type="button" onClick={cancelEditorialProposal}>
                                            Cancel
                                        </button>
                                    )}
                                    {editorialState === EditorialState.Error && lastEditorialOperation && (
                                        <button type="button" onClick={() => void requestEditorialProposal(lastEditorialOperation)}>
                                            Retry request
                                        </button>
                                    )}
                                </div>
                                {editorialMessage && (
                                    <p data-state={editorialState === EditorialState.Error ? "error" : undefined} aria-live="polite">
                                        {editorialMessage}
                                    </p>
                                )}
                                {factCheck && editorialState === EditorialState.Idle && (
                                    <section className="mt-4 border-t border-border pt-4 text-xs leading-5" aria-label="Fact check findings">
                                        <h3>Fact check</h3>
                                        <p>Advisory findings for this saved version. Missing evidence is not treated as truth.</p>
                                        {factCheck.findings.length === 0 ? (
                                            <p>No externally verifiable claims were identified.</p>
                                        ) : (
                                            <ul>
                                                {factCheck.findings.map((finding, index) => (
                                                    <li key={`${finding.claim}-${index}`} data-status={finding.status}>
                                                        <strong>
                                                            {finding.status === FACT_CHECK_STATUS.SUPPORTED
                                                                ? "Supported"
                                                                : finding.status === FACT_CHECK_STATUS.DISPUTED
                                                                  ? "Disputed"
                                                                  : "Unverifiable"}
                                                        </strong>
                            ...
                            <section className={ui.sidebarSection} aria-label="Style corpus">
                                ...
                                {styleCorpus?.profile && (
                                    <p className="mt-2 text-xs leading-5 text-muted">
                                        {styleCorpus.profile.corpusItemCount} sample(s), {styleCorpus.profile.characterCount.toLocaleString()} characters,{" "}
                                        <strong>{styleCorpus.profile.confidence}</strong> confidence.
                                    </p>
                                )}
                                {styleCorpus?.profile && (
                                    <ul className="my-3 grid list-none gap-2 p-0 text-xs">
                                        {styleCorpus.profile.traits.map((trait) => (
                                            <li key={trait.id}>
                                ...
                                <label>
                                    Sample name
                                    <input
                                        value={styleCorpusName}
                                        onChange={(event) => setStyleCorpusName(event.target.value)}
                                ...
                                <label>
                                    Article text
                                    <textarea
                                        value={styleCorpusContent}
                                        onChange={(event) => setStyleCorpusContent(event.target.value)}
                                        placeholder="Paste an author-provided article…"
                                    />
                                </label>
                                <button type="button" onClick={() => void addStyleCorpusItem()}>
                                    Add local sample
                                </button>
                                {styleCorpusMessage && <p aria-live="polite">{styleCorpusMessage}</p>}
                                {styleCorpus && (
                                    <ul className="my-3 grid list-none gap-2 p-0 text-xs">
                                        {styleCorpus.items.map((item) => (
                                            <li key={item.id}>
                                                <span>
                                                    {item.name} ({item.characterCount.toLocaleString()} characters)
                                                </span>
                                                <button type="button" onClick={() => void removeStyleCorpusItem(item.id)}>
                                                    Remove
                                                </button>
                                            </li>
                                        ))}
                            ...
                            {selected && (
                                <section className={ui.sidebarSection} aria-label="Version history">
                                            ...
                                            .map((version) => (
                                                <li key={version.id}>
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={version.id === selected.currentVersionId}
                                                    >
                                                        Restore
                                                    </button>
                                                </li>
            ...
            <section className={ui.editorPane} aria-label="Article editor">
                {selected ? (
                    <>
                        <header className="flex-wrap">
                            <div className="min-w-0">
                                ...
                                <p className="mt-1 text-xs text-muted">
                                    Version {selected.currentVersionId.slice(0, 8)} · {selected.language ?? "Source language not set"}
                                </p>
                            </div>
                            <Select
                                className="max-w-44"
                                ...
                                value={workflowStage}
                                onChange={(event) => changeWorkflowStage(event.target.value)}
                            >
                                {workflowStages.map((stage) => (
                                    <option key={stage}>{stage}</option>
                                ))}
                            </Select>
                            <p aria-live="polite" data-state={saveState}>
                                {saveState === "saving" ? "Saving…" : saveState === "error" ? "Couldn’t save. Your text is still here." : "Saved locally"}
                            </p>
                            <Button type="button" variant="quiet" onClick={toggleDistractionFree}>
                                {distractionFree ? "Show panels" : "Focus mode"}
                            </Button>
                            {saveState === "error" && (
                                <Button type="button" onClick={() => save(selected.id, draftsRef.current[selected.id] ?? "")}>
                                    Retry save
                                </Button>
                            )}
                        </header>
                        <div className="border-b border-border px-[clamp(1.5rem,5vw,5rem)] pt-3">
                            <TabList>
                        ...
                        {workspaceTab === "editor" ? (
                            <>
                                <div className="flex items-center justify-between px-[clamp(1.5rem,5vw,5rem)] pt-3 text-xs text-muted">
                                    <span>{workflowStage} is advisory. It never changes your text or runs AI.</span>
                                    <span
                                        className={
                                            publishCharacterCount > publishLimitProfile.characterLimit
                                                ? "text-danger"
                                ...
                                <textarea
                                    ...
                                    onChange={(event) => {
                                        ...
                                        setDrafts((items) => ({ ...items, [selected.id]: value }));
                                    }}
                            ...
                            <div className="mx-auto w-full max-w-4xl overflow-y-auto p-6">
                                ...
                                {!factCheck ? (
                                    <EmptyState title="No fact check yet">
                                        <p>Check facts from the assistant to create advisory findings for a saved revision.</p>
                                    </EmptyState>
                                ) : (
                                    <>
                                        <p className="mt-2 text-sm text-muted">
                                            Advisory findings for revision {proposalBase?.versionId.slice(0, 8) ?? "reviewed revision"}.{" "}
                                            {proposalIsStale
                                                ? "The article has changed since this review; findings remain attached to their original revision."
                                                : "This is the current reviewed revision."}
                                        </p>
                                        <div className="mt-4 grid gap-3">
                                            {factCheck.findings.map((finding, index) => (
                                                <article key={`${finding.claim}-${index}`} className="rounded-panel border border-border p-4">
                                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                                        <h4 className="font-semibold">Claim {index + 1}</h4>
                                                    ...
                                                    <p className="mt-2">{finding.claim}</p>
                                                    <p className="mt-2 text-sm text-muted">{finding.rationale}</p>
                                                    <p className="mt-2 text-xs text-muted">Uncertainty: {finding.uncertainty}</p>
                                                    {finding.sources.length > 0 && (
                            ...
                            <div className="mx-auto w-full max-w-4xl overflow-y-auto p-6">
                                ...
                                <p className="mt-2 text-sm text-muted">Samples remain local. Style review sends only the compact derived profile.</p>
                                {styleCorpus?.profile ? (
                                    <>
                                        <div className="mt-4 rounded-panel border border-border p-4 text-sm">
                                            <strong>{styleCorpus.profile.confidence} confidence</strong>
                                            <p className="mt-1 text-muted">
                                                {styleCorpus.profile.corpusItemCount} local sample(s) · {styleCorpus.profile.characterCount.toLocaleString()}{" "}
                                                characters
                                            </p>
                                        </div>
                                        <section className="mt-4">
                                            <h4 className="font-semibold">Profile traits</h4>
                                            <ul className="mt-2 grid gap-2">
                                                {styleCorpus.profile.traits.map((trait) => (
                                                    <li key={trait.id} className="rounded-control border border-border p-3">
                                                        <strong>{trait.label}</strong>
                                                        <p className="mt-1 text-sm text-muted">{trait.evidence}</p>
                                                    </li>
                                                ))}
                                            </ul>
                                        </section>
                                    ...
                                    <EmptyState title="No style profile">
                                        <p>Add a local sample below, then rebuild your profile through the existing local corpus flow.</p>
                                    </EmptyState>
                                )}
                                <section className="mt-5 border-t border-border pt-4">
                                    <h4 className="font-semibold">Local samples</h4>
                                    <ul className="mt-2 grid gap-2">
                                        {styleCorpus?.items.map((item) => (
                                            <li key={item.id} className="flex items-center justify-between gap-2 rounded-control border border-border p-3">
                                                <span>
                                                    {item.name} <small className="text-muted">{item.characterCount.toLocaleString()} characters</small>
                                                </span>
                                                <Button type="button" variant="danger" onClick={() => void removeStyleCorpusItem(item.id)}>
                                                    Remove
                                                </Button>
                                            </li>
                                        ))}
                                    ...
                                    <label className="mt-4 grid gap-1 text-sm">
                                        Sample name
                                        <Field value={styleCorpusName} onChange={(event) => setStyleCorpusName(event.target.value)} />
                                    </label>
                                    <label className="mt-3 grid gap-1 text-sm">
                                        Paste local sample
                                        <TextareaField value={styleCorpusContent} onChange={(event) => setStyleCorpusContent(event.target.value)} />
                                    </label>
                                    <Button type="button" className="mt-3" onClick={() => void addStyleCorpusItem()}>
                                        Add local sample
                                    </Button>
                                    {styleCorpusMessage && (
                            ...
                            <div className="m-auto w-full max-w-3xl p-8 text-sm leading-6 text-muted">
                                <h3 className="text-base font-semibold text-ink">{workspaceTabs.find((tab) => tab.id === workspaceTab)?.label}</h3>
                                <p className="mt-2">
                                    This supporting view is available in the navigation panel and does not change your article automatically.
                                </p>
                                <Button type="button" variant="secondary" className="mt-4" onClick={() => setWorkspaceTab("editor")}>
                                    Return to editor
                                </Button>
                            </div>
                        )}
                    ...
                    <EmptyState title="Select an article">
                        <p>Create an article or choose one from the sidebar.</p>
                    </EmptyState>
                )}
            </section>
            {selected && workspaceTab === "history" && (
                <section
                    className={`fixed bottom-10 top-[7.25rem] z-10 hidden overflow-hidden border-t border-border bg-surface-raised md:grid ${distractionFree ? "md:left-0" : navigationCollapsed ? "md:left-11" : "md:left-56"} ${distractionFree || assistantCollapsed ? "md:right-0" : "md:right-80"} md:grid-cols-[17rem_minmax(0,1fr)]`}
                    aria-label="Version history review"
                >
                    <aside className="overflow-y-auto border-r border-border bg-surface py-4" aria-label="Revision timeline">
                        <div className="border-b border-border px-4 pb-4">
                            <h3 className="text-sm font-semibold">Version history</h3>
                            <p className="mt-1 text-xs leading-5 text-muted">Restoring creates a new revision.</p>
                        </div>
                        {history.length === 0 ? (
                            <EmptyState title="No saved versions">
                                <p>Saved revisions will appear here.</p>
                            </EmptyState>
                        ) : (
                            <ol className="relative m-0 mt-4 grid list-none gap-1 px-3 before:absolute before:bottom-4 before:left-[1.875rem] before:top-4 before:w-px before:bg-border">
                                {history
                                    .slice()
                                    .reverse()
                                    .map((version) => (
                                        <li key={version.id} className="relative">
                                            <button
                                                type="button"
                                                className={`relative grid w-full grid-cols-[1.5rem_minmax(0,1fr)] gap-2 rounded-control p-2 text-left transition hover:bg-surface-raised ${viewedVersion?.id === version.id ? "bg-brand-soft" : ""}`}
                                                onClick={() => setViewedVersionId(version.id)}
                                            >
                                                <span
                                                    className={`z-10 mt-0.5 grid size-5 place-items-center rounded-full border text-[0.6rem] ${version.id === selected.currentVersionId ? "border-brand bg-brand text-white" : isAssistantRevision(version) ? "border-brand bg-brand-soft text-brand" : "border-border bg-surface-raised text-muted"}`}
                                                    aria-hidden="true"
                                                >
                                                    {revisionMarker(version)}
                                                </span>
                                                <span className="min-w-0">
                                                    <strong className="block truncate text-xs text-ink">
                                                        {version.id === selected.currentVersionId
                                                            ? "Current revision"
                                                            : revisionKind(version).replaceAll("-", " ")}
                                                    </strong>
                                                    <span className="mt-0.5 block text-[0.65rem] text-muted">
                                                        ...
                                                        · {countPublishingCharacters(version.content).toLocaleString()} chars
                                                    </span>
                                                </span>
                                            </button>
                                        </li>
                    ...
                    <article className="flex min-w-0 flex-col overflow-hidden" aria-label="Revision preview">
                        {viewedVersion ? (
                            <>
                                <header className="flex min-h-16 items-center gap-4 border-b border-border px-6 py-3">
                                    <div className="min-w-0">
                                        <h3 className="truncate text-base font-semibold">
                                            {viewedVersion.id === selected.currentVersionId ? "Current article revision" : "Saved article revision"}
                                        </h3>
                                        <p className="mt-1 text-xs text-muted">
                                            {typeof viewedVersion.provenance.kind === "string"
                                                ? viewedVersion.provenance.kind.replaceAll("-", " ")
                                                : "saved revision"}{" "}
                                            · {new Date(viewedVersion.createdAt).toLocaleString()}
                                        </p>
                                    </div>
                                    {viewedVersion.id === selected.currentVersionId ? (
                                        <span className="ml-auto shrink-0 text-xs font-semibold text-brand">
                                            Current version
                                        </span>
                                    ) : (
                                        <Button
                                            ...
                                            variant="secondary"
                                            onClick={() => void restoreVersion(viewedVersion)}
                                        >
                                            ↶ Restore this version
                                        </Button>
                                    )}
                                </header>
                                <pre className="mx-auto w-full max-w-[52rem] flex-1 overflow-y-auto whitespace-pre-wrap px-8 py-8 font-editor text-[1.05rem] leading-8 text-ink">
                                    {viewedVersion.content}
                                </pre>
                            </>
                        ) : (
                            <EmptyState title="No version selected">
                                <p>Select a revision from the timeline.</p>
                            </EmptyState>
                        )}
                    </article>
                </section>
            ...
            {selected && (
                <footer
                    className={`fixed inset-x-0 bottom-0 z-20 hidden h-10 items-center border-t border-border bg-surface-raised px-5 text-xs text-muted shadow-[0_-1px_2px_rgb(37_37_33_/_3%)] md:flex ${distractionFree ? "md:left-0" : navigationCollapsed ? "md:left-11" : "md:left-56"} ${distractionFree || assistantCollapsed ? "md:right-0" : "md:right-80"}`}
                ...
                >
                    ...
                    <span>
                        Version {selected.currentVersionId.slice(0, 8)} ·{" "}
                        {new Date(selected.updatedAt).toLocaleString([], { hour: "2-digit", minute: "2-digit" })}
                    </span>
            ...
            {!distractionFree && (
                <aside
                    className={`${assistantCollapsed ? "hidden" : ""} min-w-0 overflow-y-auto border-l border-border bg-surface-raised px-4 py-4`}
                    aria-label="Editorial assistant"
                >
                    <div className="flex items-center justify-between gap-2 border-b border-border pb-4">
                        <div>
                            <h2 className="text-sm font-semibold">✦&nbsp; AI Assistant</h2>
                            <p className="mt-1 text-[0.7rem] text-muted">Suggestions require your approval.</p>
                        </div>
                        <Button type="button" variant="quiet" className="min-h-8 px-2 py-1" aria-label="Collapse assistant" onClick={toggleAssistantPanel}>
                            ›
                        </Button>
                    </div>
                    <div className="mt-4 rounded-panel bg-brand p-3 text-xs leading-5 text-white">
                        <strong>Flow &amp; clarity review</strong>
                        <p className="mt-1 text-white/90">Review this article and suggest improvements to make it more cohesive.</p>
                    </div>
                    <div className="mt-4 rounded-panel border border-border bg-canvas p-3 text-xs leading-5 text-muted">
                        <strong className="text-ink">Input scope</strong>
                        <p className="mt-1">
                            Sends the saved revision {selected ? selected.currentVersionId.slice(0, 8) : "—"} and only the guidance you enter below.
                        </p>
                    </div>
                    <label className="mt-4 grid gap-1 text-xs text-muted">
                        ...
                        <TextareaField
                            value={editorialContext}
                            onChange={(event) => setEditorialContext(event.target.value)}
                            placeholder="Add talking points or revision guidance…"
                            disabled={!selected || editorialState === EditorialState.Streaming}
                        />
                    </label>
                    ...
                    {editorialState === EditorialState.Streaming && (
                        <Button type="button" variant="danger" className="mt-3 w-full" onClick={cancelEditorialProposal}>
                            Stop request
                        </Button>
                    )}
                    {editorialState === EditorialState.Error && lastEditorialOperation && (
                        <Button type="button" className="mt-3 w-full" onClick={() => void requestEditorialProposal(lastEditorialOperation)}>
                            Retry request
                        </Button>
                    )}
                    {editorialMessage && (
                        <p className={`mt-3 text-xs leading-5 ${editorialState === EditorialState.Error ? "text-danger" : "text-muted"}`} aria-live="polite">
                            {editorialMessage}
                        </p>
                    )}
                </aside>
            )}
            {!distractionFree && assistantCollapsed && (
                <Button type="button" className="fixed bottom-4 right-4 shadow-raised" onClick={toggleAssistantPanel}>
                    Open assistant
                </Button>
            )}
            <Dialog open={Boolean(restoreCandidate)} aria-labelledby="restore-version-title">
                {restoreCandidate && (
                    <div className="grid w-[min(39rem,calc(100vw-2rem))] gap-5">
                        <div>
                            <h2 id="restore-version-title" className="text-lg font-semibold">
                                Restore this version?
                            </h2>
                            <p className="mt-3 text-base leading-7 text-ink">
                                This will create a new revision in the history with the content from <strong>{revisionKind(restoreCandidate).replaceAll("-", " ")}</strong>.
                            </p>
                        </div>
                        ...
                        <div className="flex justify-end gap-3">
                            <Button type="button" variant="secondary" onClick={() => setRestoreCandidate(undefined)}>
                                Cancel
                            </Button>
                            <Button type="button" onClick={() => void restoreVersion(restoreCandidate)}>
                                Restore
                            </Button>
                        </div>
                    </div>
            ...
            <Dialog open={isCreateDialogOpen} aria-labelledby="new-document-title">
                <form method="dialog" className="grid w-[min(32rem,calc(100vw-2rem))] gap-4">
                    <div>
                        <h2 id="new-document-title" className="text-lg font-semibold">
                            New document
                        </h2>
                        <p className="mt-1 text-sm text-muted">Start with an empty draft or save your talking points for a reviewable narrative proposal.</p>
                    </div>
                    <label className="grid gap-1 text-sm">
                        Title
                        <Field value={newDocumentTitle} onChange={(event) => setNewDocumentTitle(event.target.value)} placeholder="Article title" autoFocus />
                    </label>
                    <label className="grid gap-1 text-sm">
                        Talking points <span className="text-xs text-muted">(optional)</span>
                        <TextareaField
                            value={newDocumentPoints}
                            onChange={(event) => setNewDocumentPoints(event.target.value)}
                            placeholder="Claims, structure, links, or notes…"
                        />
                    </label>
                    <div className="grid gap-4 sm:grid-cols-2">
                        <label className="grid gap-1 text-sm">
                            Source language
                            <Field value={newDocumentLanguage} onChange={(event) => setNewDocumentLanguage(event.target.value)} />
                        </label>
                        <label className="grid gap-1 text-sm">
                            Audience <span className="text-xs text-muted">(optional)</span>
                            <Field
                                value={newDocumentAudience}
                                onChange={(event) => setNewDocumentAudience(event.target.value)}
                    ...
                    <label className="grid gap-1 text-sm">
                        Publishing profile
                        <Select value={publishLimitProfileId} onChange={(event) => setPublishLimitProfileId(event.target.value as PublishLimitProfileId)}>
                            {publishLimitProfiles.map((profile) => (
                                <option key={profile.id} value={profile.id}>
                                    {profile.label}
                                </option>
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                        <Button type="button" variant="quiet" onClick={() => setIsCreateDialogOpen(false)}>
                            Cancel
                        </Button>
                        <Button type="button" variant="secondary" onClick={() => void createDocumentFromFlow(false)}>
                            Create empty draft
                        </Button>
                        <Button type="button" disabled={!newDocumentPoints.trim()} onClick={() => void createDocumentFromFlow(true)}>
                            Save talking points
                        </Button>
                    </div>
                </form>
