...
export class HttpApplicationClient implements ApplicationClient, WorkspaceClient {
  ...
  private async request<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    if (response.status === 409 && typeof body === "object" && body !== null && "document" in body) {
      throw new DocumentConflictError((body as { document: Document }).document);
    }
    if (!response.ok) {
      const message = typeof body === "object" && body !== null && "error" in body && typeof body.error === "string" ? body.error : `The local service could not be reached (${response.status}).`;
