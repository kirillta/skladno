...
export class ConfiguredEditorialEngineResolver implements EditorialEngineResolver {
    ...
    resolve(operation: EditorialOperation, assistantSkillId?: BuiltInSkillId): EditorialEngine | undefined {
        const savedConnections = this.settings.getSetting("application-ai-connections")?.value as { connections?: OpenAiConnection[]; activeConnectionId?: string } | undefined;
        ...
        const preferences = this.settings.getSetting("application-model-preferences")?.value as Partial<ModelPreferences> | undefined;
