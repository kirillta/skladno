...
test("allows manual model IDs only for basic text generation", () => {
    ...
    assert.equal(service.getCapabilities(AI_PROVIDER.OPENAI, "gpt-5").sourcedResearch, true);
    assert.equal(service.getCapabilities(AI_PROVIDER.ANTHROPIC, "claude-sonnet").sourcedResearch, false);
