...
export class DocumentsRepository {
    ...
    create(input: CreateDocumentInput): Document {
        ...
        try {
            this.database.prepare("INSERT INTO documents (id, title, language, audience, publishing_profile_id, source_document_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
                .run(documentId, required(input.title, "Document title"), input.language ?? null, input.audience ?? null, input.publishingProfileId ?? null, input.sourceDocumentId ?? null, timestamp, timestamp);
    ...
    list(): Document[] {
        return (this.database.prepare("SELECT d.id document_id, d.title, d.language, d.audience, d.publishing_profile_id, d.source_document_id, d.created_at document_created_at, d.updated_at document_updated_at, v.* FROM documents d JOIN document_versions v ON v.id = d.current_version_id ORDER BY d.updated_at DESC, d.id ASC").all() as Row[]).map(document);
    }


    get(documentId: string): Document | undefined {
        const row = this.database.prepare("SELECT d.id document_id, d.title, d.language, d.audience, d.publishing_profile_id, d.source_document_id, d.created_at document_created_at, d.updated_at document_updated_at, v.* FROM documents d JOIN document_versions v ON v.id = d.current_version_id WHERE d.id = ?").get(documentId) as Row | undefined;
