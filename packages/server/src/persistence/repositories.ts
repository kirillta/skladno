...
export class Repositories {
    ...
    createMaterial(input: CreateMaterialInput): Material {
        const timestamp = now(); 
        const materialId = input.id ?? id();

        this.database
            .prepare("INSERT INTO materials (id, name, content, created_at, updated_at) VALUES (?, ?, ?, ?, ?)")
    ...
    createDocument(input: CreateDocumentInput): Document {
        ...
        try {
            ...
            this.database
                .prepare("INSERT INTO document_versions (id, document_id, content, provenance_json, created_at) VALUES (?, ?, ?, ?, ?)")
    ...
    listDocuments(): Document[] { 
        return (this.database
            .prepare(`SELECT d.id document_id, d.title, d.created_at document_created_at, d.updated_at document_updated_at, v.* FROM documents d JOIN document_versions v ON v.id = d.current_version_id ORDER BY d.updated_at DESC, d.id ASC`)
            .all() as Row[])
            .map(document); 
    }
    ...
    getDocument(documentId: string): Document | undefined { 
        const row = this.database
            .prepare(`SELECT d.id document_id, d.title, d.created_at document_created_at, d.updated_at document_updated_at, v.* FROM documents d JOIN document_versions v ON v.id = d.current_version_id WHERE d.id = ?`)
    ...
    listVersions(documentId: string): DocumentVersion[] { 
        return (this.database
            .prepare("SELECT * FROM document_versions WHERE document_id = ? ORDER BY created_at ASC, id ASC")
    ...
    private appendVersion(documentId: string, content: string, provenance: Record<string, unknown>, restoredFromVersionId?: string): DocumentVersion {
        ...
        try { 
            this.database
                .prepare("INSERT INTO document_versions (id, document_id, content, provenance_json, restored_from_version_id, created_at) VALUES (?, ?, ?, ?, ?, ?)")
        ...
        return this.listVersions(documentId)
    ...
    restoreVersion(documentId: string, historicalVersionId: string): DocumentVersion {
        const historical = this.database
            ...
            .get(historicalVersionId, documentId) as Row | undefined;
    ...
    createWorkflowArtifact(input: CreateWorkflowArtifactInput): WorkflowArtifact {
        const artifactId = input.id ?? id(), timestamp = now(); 
        required(input.kind, "Artifact kind");

        this.database
            .prepare("INSERT INTO workflow_artifacts (id, document_id, version_id, kind, content, created_at) VALUES (?, ?, ?, ?, ?, ?)")
            .run(artifactId, input.documentId, input.versionId, input.kind, input.content, timestamp);
    ...
    createSourceCitation(input: CreateSourceCitationInput): SourceCitation {
        const citationId = input.id ?? id(), timestamp = now(); 
        required(input.url, "Citation URL");

        this.database
            .prepare("INSERT INTO source_citations (id, artifact_id, url, title, excerpt, uncertainty, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)")
            .run(citationId, input.artifactId, input.url, input.title ?? null, input.excerpt ?? null, input.uncertainty ?? null, timestamp);
    ...
    setSetting(key: string, value: unknown): AppSetting { 
        ...
        const updatedAt = now(); 
        this.database
            .prepare("INSERT INTO app_settings (key, value_json, updated_at) VALUES (?, ?, ?) ON CONFLICT(key) DO UPDATE SET value_json = excluded.value_json, updated_at = excluded.updated_at")
