...
- Select editorial operations through one **Quick actions** dropdown button immediately above the composer.
- Open Quick actions as a compact floating menu upward so it does not reflow or resize the composer. Do not use a permanently expanded grid, a collapsible in-flow button stack, or a native select.
- Make a Quick action select the pending editorial operation without sending the request. Keep the selected operation visible in the Quick actions button.
...
- Keep the target-language selector in the Article Header alongside article-level controls, not in the Assistant Panel.
