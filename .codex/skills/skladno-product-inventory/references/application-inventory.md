...
| Article header | Rename, workflow, source/target language, save Revision, focus mode, delete | Implemented | `ArticleHeader`; workflow and source persist as Article metadata, while target language is stable request guidance only. Ordinary Draft checkpoint save state is intentionally owned by the visible Article Library Panel or Navigation Rail for MVP. |
