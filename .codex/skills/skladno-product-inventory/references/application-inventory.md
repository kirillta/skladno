...
| Article library | Create, search, select, and list recent Articles | Implemented | `ArticleLibraryPanel` and Article routes; new Articles use the configured default Article language. Articles are newest-work-first by the later of Article metadata/Revision activity and any retained Draft checkpoint. |
...
| Article header | Rename, source/target language, save Revision, focus mode, delete | Implemented | `ArticleHeader`; source persists as Article metadata, while target language is stable request guidance only. Ordinary Draft checkpoint save state is intentionally owned by the visible Article Library Panel or Navigation Rail for MVP. |
| Editorial Assistant Panel | Persistent conversation, optional built-in skills, Quick actions, and selection-scoped requests | Partial | `EditorialAssistantPanel` and Assistant request contracts; each Article restores its local timeline, requests remain explicit, and selected text is restricted to compatible skills. Retry and richer request-progress presentation remain follow-up work. |
| Workspace navigation | Write, Proposal Review, Revisions, Fact Check, Style Profile, Translations, Publish with revision-aware actionable and stale badges | Implemented | `WorkspaceTabBar` and `WorkspaceViewRouter`; active view and selected Article are local workspace preferences restored after reload, while each editorial result retains its own Article and base Revision for freshness. |
...
| Article status | Current Revision, Article publishing-profile selector, plus shared count/limit and near/over guidance | Implemented | `ArticleStatusBar`; fixed 24px bottom row of the Article Workspace. The accepted MVP does not add remaining/overflow wording. |
