...
| Assistant persistence | Local Article conversation records | Partial | Greetings, messages, and requests are isolated per Article with cascade deletion; fixed greetings use a stable template for interface-localized rendering, while authored and model content remain persisted text. |
