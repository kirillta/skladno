...
| `skills/file-assistant-skill-source.ts`, `author-skill-service.ts`, `infrastructure/skills/skill-revision-store.ts` | Refresh, managed create/update/restore/delete, immutable snapshots and rollback on a failed snapshot write; transient staging is excluded and normalized duplicate names are rejected | Add typed errors, crash journal/recovery and serialized mutations; current `.previous` is deleted after replacement |
...
| `application/create-application-services.ts` and its options | Optional Author root, lifecycle service and file-backed Revision store composed beneath the active data root | Expose only after journal/recovery checks pass |
