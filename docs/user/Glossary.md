...
- **Skill**: a reusable instruction package that helps Assistant choose and sequence Editorial capabilities for an Author's request. A Skill grants no capability or permission. A **built-in Skill** ships with Skladno; an **Author Skill** is created and managed by an Author.
